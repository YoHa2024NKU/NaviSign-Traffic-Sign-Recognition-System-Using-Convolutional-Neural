...
def toggle_video():
    ...
    if not video_running:
        ...
        if cap:
            cap.release()
            cap = None
        sign_image.configure(image='')  # Clear the video frame
        sign_image.image = None  # Reset the image reference
...
heading = Label(top, text="Real Time Traffic Sign Recognition", pady=20, font=('arial', 20, 'bold'))
